...
createBall =
  ...
  in object "ball" ballPic False (w/2,h-10) (0,-4) ()

createBall2 :: GameObject ()
createBall2 =
  let ballPic = Basic (Circle 17.0 0.0 1.0 0.0 Filled)
  in object "ball2" ballPic False (w-50,h-50) (0,-8) ()

createBall3 :: GameObject ()
createBall3 =
  let ballPic = Basic (Circle 17.0 0.0 1.0 0.0 Filled)
  in object "ball3" ballPic False ((w/2)+60,h-50) (0,-6) ()

createBall4 :: GameObject ()
createBall4 =
  let ballPic = Basic (Circle 17.0 0.0 1.0 0.0 Filled)
  in object "ball4" ballPic False (50,h-50) (0,-7) ()

createBall5 :: GameObject ()
createBall5 =
  let ballPic = Basic (Circle 17.0 0.0 1.0 0.0 Filled)
  in object "ball5" ballPic False ((w/2)-50,h-50) (0,-6) ()
...
gameCycle = do
  ...
  when col4  ( do
    setObjectPosition ((w/2),h-10)   ball
    setGameAttribute (Score (n+1))
       
              )
  when (n>=10) (do
      setObjectPosition (0,1000) ball
    )
  when (n>=20)(do
      setObjectPosition (0,1000) ball2
    )
 
  when (n>=30) (do
      setObjectPosition (0,1000) ball3
    )
  when (n>=35)(do
      setObjectPosition (0,1000) ball4
    )
  when (n>=40)(do
      setObjectPosition (0,1000) ball5

    )   

  col6 <- objectBottomMapCollision ball2
  when col6 ( do
      setObjectPosition ((w-50),h-50) ball2
      setGameAttribute (Score (n+1))

              )
  col8 <- objectBottomMapCollision ball3
  when col8 (do
      setObjectPosition ((w/2)+50,h-50) ball3
      setGameAttribute (Score (n+1))
            )
